...
function Library:change_theme(toTheme)
	...
	math.floor(c.R * 255) ..
	"," .. math.floor(c.G * 255) .. "," .. math.floor(c.B * 255) ..
	...
	function methods:fade(state, colorOverride, length, instant)
		...
		if not rawget(self, "fadeFrame") then
			local frame = self:object("Frame", {
				...
				Centered = true,
				ZIndex = 999
			}):round(self.AbsoluteObject:FindFirstChildOfClass("UICorner") and
			self.AbsoluteObject:FindFirstChildOfClass("UICorner").CornerRadius.Offset or 0)
...
function Library:create(options)
			...
			Event:connect(function()
				local Input = core.InputBegan:connect(function(Key)
					if Key.UserInputType == Enum.UserInputType.MouseButton1 then
						local ObjectPosition = Vector2.new(Mouse.X - core.AbsolutePosition.X,
							Mouse.Y - core.AbsolutePosition.Y)
						while RunService.RenderStepped:wait() and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
							if Library.LockDragging then
								local FrameX, FrameY =
								math.clamp(Mouse.X - ObjectPosition.X, 0, gui.AbsoluteSize.X - core.AbsoluteSize.X),
									math.clamp(Mouse.Y - ObjectPosition.Y, 0, gui.AbsoluteSize.Y - core.AbsoluteSize.Y)
								core:tween {
	...
	local profilePictureContainer = profile:object("ImageLabel", {
		Image = Players:GetUserThumbnailAsync(LocalPlayer.UserId, Enum.ThumbnailType.HeadShot,
	...
	creditsTab:credit { Name = "Synax", Description = "Cloudy Hub Developer", Discord = "damionb" }
...
function Library:notification(options)
	...
	noti:tween({ BackgroundTransparency = 0, Length = 0.2, Size = UDim2.fromOffset(300, text.TextBounds.Y + 63) },
		function()
...
function Library:tab(options)
	...
	tabButtonClose.MouseButton1Click:connect(function()
		...
		tabButton:tween({ Size = UDim2.new(0, 50, tabButton.Size.Y.Scale, tabButton.Size.Y.Offset), Length = 0.1 },
			function()
		...
		for _, v in next, fitems do
			for _2, v2 in next, items do
				...
				if label:lower() == tostring(v):lower() then
					...
					dropdownContainer:tween({ Size = (not open and UDim2.new(1, -20, 0, 52)) or
					UDim2.new(1, -20, 0, 52 + newSize) })
				end
			end
		end
	end
				...
				do
					...
					do
						updateHue = function()
							...
							local tempVal = math.clamp(
							(Mouse.Y - pickerArea.AbsolutePosition.Y) / (pickerArea.AbsoluteSize.Y), 0, 1)
							local newYPos = math.clamp(
							(Mouse.Y - hueBar.AbsolutePosition.Y) / (hueBar.AbsoluteSize.Y) * hueBar.AbsoluteSize.Y, 0,
								hueBar.AbsoluteSize.Y)
							...
							hueDraggable:tween({ Length = 0.05, Position = UDim2.new(-2, 3, 0,
								math.clamp(newYPos - 10, -10, hueBar.AbsoluteSize.Y + 10)), ImageColor3 = Color3.fromHSV(
							1, 0, -tempVal) })
						end
					...
					do
						...
						updatePicker = function()
							sat = math.clamp((Mouse.X - pickerArea.AbsolutePosition.X) / (pickerArea.AbsoluteSize.X), 0,
								1)
							val = 1 -
							math.clamp((Mouse.Y - pickerArea.AbsolutePosition.Y) / (pickerArea.AbsoluteSize.Y), 0, 1)

							local newXPos = math.clamp(
							(Mouse.X - pickerArea.AbsolutePosition.X) / (pickerArea.AbsoluteSize.X) *
							pickerArea.AbsoluteSize.X, 0, pickerArea.AbsoluteSize.X)
							local newYPos = math.clamp(
							(Mouse.Y - pickerArea.AbsolutePosition.Y) / (pickerArea.AbsoluteSize.Y) *
							pickerArea.AbsoluteSize.Y, 0, pickerArea.AbsoluteSize.Y)
				...
				do
					...
					do
						updateHue = function()
							...
							local newXPos = math.clamp(
							(Mouse.X - hueArea.AbsolutePosition.X) / (hueArea.AbsoluteSize.X) * hueArea.AbsoluteSize.X, 0,
								hueArea.AbsoluteSize.X)
							selectedColor = Color3.fromHSV(hue, sat, val)
							color:tween({ Length = 0.05, BackgroundColor3 = Color3.fromHSV(hue, 1, 1) })
							hueDraggable:tween({ Length = 0.05, Position = UDim2.new(0,
								math.clamp(newXPos, 0, hueArea.AbsoluteSize.X), .5, 0) })
	...
	do
		...
		keybindContainer.MouseButton1Click:connect(function()
			if not listening then
				listening = true; keybindDisplay.Text = "..."
			end
		end)
	end
	self:_resize_tab()
